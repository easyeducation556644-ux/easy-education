...
function getOperationsServiceAccount() {
  if (process.env.OPS_FIREBASE_SERVICE_ACCOUNT) {
    return JSON.parse(process.env.OPS_FIREBASE_SERVICE_ACCOUNT)
  }
  if (
    process.env.OPS_FIREBASE_PROJECT_ID &&
    process.env.OPS_FIREBASE_CLIENT_EMAIL &&
    process.env.OPS_FIREBASE_PRIVATE_KEY
  ) {
    return {
