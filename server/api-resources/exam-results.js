...
async function save(authenticated, req, res) {
  ...
  const body = req.body || {}
  const courseId = text(body.courseId).replace(/^cps:/, "")
...
export default async function handler(req, res) {
  ...
  try {
    ...
    res.status(Number(error?.statusCode) || 500).json({ error: error?.message || "Exam result service is temporarily unavailable", code: error?.code || "EXAM_RESULT_ERROR" })
  }
}
